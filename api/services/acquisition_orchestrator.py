...
For each ``running`` :class:`AcquisitionRun`, this worker advances its items one
step per tick through the state machine::

    found → (auto_enrich) → enriched → (auto_generate) → generated
          → (semi_auto: PAUSE review) → (auto_campaign) → campaigning

It only orchestrates — every actual action reuses an existing unit service
(``enrichment_service``, ``demo_site_service``, ``campaign_service`` /
``CampaignQueueService``).  All state lives in the database, so the worker is
stateless: a crash or restart resumes exactly where it stopped, exactly like the
cold-email queue worker (``email_queue_worker``).

Guardrails enforced here (honest, no silent caps):
  * **Credit floor** — a non-admin user with a 0 balance can't run costly steps;
    the run pauses with a clear reason instead of failing prospect by prospect.
  * **Per-run credit budget** (``max_credits``) — measured as the delta of the
    user's consumed credits since the run started; the run pauses when reached.
  * **Daily email cap** (``daily_email_cap``) — caps how many prospects a single
    campaign launch enqueues; the overflow is skipped with a visible reason.
"""
...
_TICK_INTERVAL_SECONDS: int = 30
#: Max item-advances performed per tick across all runs (keeps a huge run from
#: monopolising a tick; the next tick continues where this one stopped).
_MAX_ADVANCES_PER_TICK: int = 8
#: How many times a transient step (enrich/generate) is retried before failing.
_MAX_ITEM_ATTEMPTS: int = 3
...
async def run_acquisition_loop() -> None:
    """Background loop: advance running acquisition sequences, one step at a time."""
    logger.info("[Acquisition] Orchestrator started — tick=%ds", _TICK_INTERVAL_SECONDS)
    while True:
        try:
            await _tick()
...
async def _tick() -> None:
    ...
    try:
        ...
        budget: int = _MAX_ADVANCES_PER_TICK
        for run in runs:
            if budget <= 0:
                break
            try:
                used: int = await _advance_run(db, run, budget)
                budget -= used
...
async def _advance_run(db: Session, run, budget: int) -> int:
    """
    ...
    if active_items:
        ...
        if balance == 0:
            ...
            _merge_stats(run, {"pause_reason": "crédits épuisés"})
            db.commit()
        ...
        if run.max_credits is not None:
            ...
            if baseline is not None:
                ...
                if spent >= run.max_credits:
                    run.status = AcquisitionRunStatus.PAUSED.value
                    _merge_stats(run, {"pause_reason": "plafond de crédits atteint"})
                    db.commit()
    ...
    for item in active_items:
        if used >= budget:
            break
        await _advance_item(db, run, item, user)
        used += 1

    # --- Reconcile run status after the generation phase --------------------
    if not any(i.step in _active_steps(run) for i in run.items):
        ...
        if run.auto_campaign and generated:
            if (
                ...
                await _launch_campaign(db, run, user, generated)
...
async def _advance_item(db: Session, run, item, user) -> None:
    ...
    if prospect is None:
        _fail(db, item, "prospect introuvable")
        return
    ...
    if prospect.reserved_by_user_id not in (None, run.user_id):
        _skip(db, item, "réservé par un autre membre")
        return

    step: str = item.step

    if step == AcquisitionItemStep.FOUND.value:
        await _do_enrich(db, run, item, user, prospect)
    elif step == AcquisitionItemStep.ENRICHED.value:
        await _do_generate(db, run, item, user, prospect)
    else:
...
async def _do_enrich(db: Session, run, item, user, prospect) -> None:
    """FOUND → ENRICHED (running enrichment when ``auto_enrich``)."""
    from enums.acquisition import AcquisitionItemStep
    from services.enrichment_service import enrichment_service

    if not run.auto_enrich:
        _set_step(db, item, AcquisitionItemStep.ENRICHED.value)
        return
    ...
    try:
        ...
        _retry_or_fail(db, item, f"enrichissement: {exc}")
        return

    # Poor enrichment still advances (the site will just be sparser) but is flagged.
    reason: Optional[str] = None
    if enrichment is None or not _enrichment_has_signal(enrichment):
        reason = "enrichissement pauvre"
    _set_step(db, item, AcquisitionItemStep.ENRICHED.value, reason)


async def _do_generate(db: Session, run, item, user, prospect) -> None:
    ...
    if not prospect.email:
        _skip(db, item, "pas d'email — site non généré")
        return
    ...
    try:
        ...
        _skip(db, item, f"génération impossible: {exc}")
        return
    except Exception as exc:  # noqa: BLE001
        _retry_or_fail(db, item, f"génération: {exc}")
        return
...
async def _launch_campaign(db: Session, run, user, generated: List) -> None:
    """
    ...
    for item in generated:
        prospect: Optional[ProspectDB] = db.get(ProspectDB, item.prospect_id)
        if prospect is None:
            _skip(db, item, "prospect introuvable")
            continue
        if prospect.user_id != run.user_id:
            _skip(db, item, "appartient à un autre membre")
            continue
        if prospect.reserved_by_user_id not in (None, run.user_id):
            _skip(db, item, "réservé par un autre membre")
            continue
        if not prospect.email:
            _skip(db, item, "pas d'email")
            continue
        eligible.append(item)

    if not eligible:
        _merge_stats(run, {"campaign_note": "aucun prospect éligible à démarcher"})
        return
    ...
    if run.daily_email_cap is not None and len(eligible) > run.daily_email_cap:
        for item in eligible[run.daily_email_cap:]:
            _skip(db, item, "plafond quotidien atteint")
    ...
    if not run.email_template_id_a:
        ...
        _merge_stats(run, {"campaign_note": "aucun modèle d'email — démarchage manuel requis"})
        return
    ...
    _create_follow_ups(db, campaign.id, run.follow_ups)
    ...
    has_resend: bool = (
        ...
        is not None
    ) and _resend_has_key(db, run.user_id)

    if not has_resend:
        # Campaign built but not sending — mark items campaigning, note the gap.
        for item in eligible:
            _set_step(db, item, AcquisitionItemStep.CAMPAIGNING.value, "en attente — Resend non configuré")
        _merge_stats(run, {"campaign_note": "Resend non configuré — campagne créée en brouillon"})
    ...
    for item in eligible:
        if item.prospect_id in skipped_ids:
            _set_step(db, item, AcquisitionItemStep.SKIPPED.value, "site de démo inactif")
        else:
            _set_step(db, item, AcquisitionItemStep.CAMPAIGNING.value)

    _merge_stats(run, {"campaign_note": f"{result.enqueued} email(s) en file"})
    logger.info(
...
def _create_follow_ups(db: Session, campaign_id: int, follow_ups: Optional[list]) -> None:
...
def _resend_has_key(db: Session, user_id: int) -> bool:
...
def _enrichment_has_signal(enrichment) -> bool:
    """
...
def _set_step(db: Session, item, step: str, reason: Optional[str] = None) -> None:
...
def _skip(db: Session, item, reason: str) -> None:
...
def _fail(db: Session, item, reason: str) -> None:
...
def _retry_or_fail(db: Session, item, error: str) -> None:
    """
    Increment the attempt counter; keep the item retryable until it exhausts
    ``_MAX_ITEM_ATTEMPTS``, then fail it terminally.
    """
    from enums.acquisition import AcquisitionItemStep

    item.attempts += 1
    item.last_error = error
    if item.attempts >= _MAX_ITEM_ATTEMPTS:
        item.step = AcquisitionItemStep.FAILED.value
...
def _merge_stats(run, patch: dict) -> None:
