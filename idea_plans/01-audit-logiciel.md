...
- ✅ **Rendu des `social_links` sur les sites** : ~~pas encore de section réseaux sociaux~~ → **branché de bout en bout** — champ `social` ajouté au contrat partagé `SiteContent` (`@devleadhunter/website-content` **v1.3.0**), mappé par l'API (`map_prospect_and_enrichment` + schéma Storyblok + bridges), rendu dans le **footer du template par défaut** (artisan-edito **v1.2.0**, liens vérifiés en SSR), bridge demo-host + pin bumpés (typecheck vert). *(Reste : répliquer le rendu social aux 4 autres templates — le champ est déjà dispo pour eux.)*
