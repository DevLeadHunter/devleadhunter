//! Prod MySQL/MariaDB → local dev database sync (read-only dump → import). Desktop debug builds only.
//!
//! Uses host `mysqldump` / `mysql` when available, otherwise **`docker run mariadb:11`**
//! (`mariadb-dump` / `mariadb` in the image, protocol-compatible with MySQL) when
//! `DLH_SYNC_USE_DOCKER=auto` and no local clients are on PATH.
...
/// Load `api/.env.sync` then `web/.env.sync` (both gitignored) so the sync credentials
/// never live in the committed config. Later files override earlier ones.
    ...
    for p in paths {
        if p.is_file() {
            if let Err(e) = dotenvy::from_path(&p) {
                eprintln!("[DevLeadHunter] Failed to load .env.sync {:?}: {}", p, e);
            }
        }
    }
}
...
enum SyncBackend {
    Host { mysqldump: String, mysql: String },
    ...
    match mode {
        ...
        "never" => {
            if !client_version_ok(mysqldump_bin) {
                return Err(format!(
                    "`{mysqldump_bin}` not found (DLH_SYNC_USE_DOCKER=never)."
                ));
            }
            if !client_version_ok(mysql_bin) {
                return Err(format!(
                    "`{mysql_bin}` not found (DLH_SYNC_USE_DOCKER=never)."
                ));
            }
            Ok(SyncBackend::Host {
        ...
        _ => {
            ...
            Err(
                "Neither MySQL/MariaDB clients (mysqldump / mysql) nor Docker is usable. \
                 Install Docker Desktop (https://www.docker.com/products/docker-desktop/) \
                 or MySQL/MariaDB client binaries on PATH."
                    .into(),
            )
        }
    }
...
fn run_mysqldump(backend: &SyncBackend, prod_cnf: &Path, prod_db: &str) -> Result<Vec<u8>, String> {
    match backend {
        SyncBackend::Host { mysqldump, .. } => {
            ...
            let dump_out = command_hidden(mysqldump)
                .arg(&prod_extra)
                .args(["--single-transaction", "--skip-lock-tables", "--skip-triggers"])
                .arg(prod_db)
                .stderr(Stdio::piped())
                .stdout(Stdio::piped())
                .output()
                .map_err(|e| {
                    format!("Could not run `{mysqldump}`: {e}. Use Docker or set DLH_MYSQLDUMP_BIN.")
                })?;
        ...
        SyncBackend::Docker { docker, image } => {
            let mount = docker_volume_mount(prod_cnf, "/tmp/dlh_sync_prod.cnf")?;
            let dump_out = command_hidden(docker)
                .args([
                    ...
                    image.as_str(),
                    "mariadb-dump",
                    "--defaults-extra-file=/tmp/dlh_sync_prod.cnf",
                    "--single-transaction",
                    "--skip-lock-tables",
                    "--skip-triggers",
                    prod_db,
                ...
                .output()
                .map_err(|e| {
                    format!("`docker run … mariadb-dump` failed: {e}. Check Docker Desktop and image `{image}`.")
                })?;
            if !dump_out.status.success() {
                let err = String::from_utf8_lossy(&dump_out.stderr);
...
fn run_mysql_exec(backend: &SyncBackend, local_cnf: &Path, args: &[&str]) -> Result<(), String> {
    match backend {
        SyncBackend::Host { mysql, .. } => {
            ...
            let out = cmd
                .stderr(Stdio::piped())
                .output()
                .map_err(|e| format!("Could not run `{mysql}`: {e}. Use Docker or set DLH_MYSQL_BIN."))?;
        ...
        SyncBackend::Docker { docker, image } => {
            let mount = docker_volume_mount(local_cnf, "/tmp/dlh_sync_local.cnf")?;
            ...
            cmd.args([
                ...
                image.as_str(),
                "mariadb",
                "--defaults-extra-file=/tmp/dlh_sync_local.cnf",
            ]);
            for a in args {
                cmd.arg(a);
            }
            let out = cmd
                .stderr(Stdio::piped())
                .output()
                .map_err(|e| format!("`docker run … mariadb`: {e}"))?;
            if !out.status.success() {
                return Err(String::from_utf8_lossy(&out.stderr).to_string());
            }
            Ok(())
...
/// Dump prod to SQL, then import into the local database (DROP TABLE from the dump replaces
/// existing local tables). Credentials are read from `.env.sync` and written to temp `.cnf`
/// files (never on the command line), removed via a `Drop` guard.
pub fn sync_dev_database_from_prod_impl() -> Result<String, String> {
    load_env_sync_files();

    let prod_host = req_env("DLH_SYNC_PROD_HOST")?;
    let prod_port = env_u16("DLH_SYNC_PROD_PORT", 3306);
    let prod_user = req_env("DLH_SYNC_PROD_USER")?;
    let prod_pass = req_env("DLH_SYNC_PROD_PASSWORD")?;
    let prod_db = sanitize_db_name(&req_env("DLH_SYNC_PROD_DATABASE")?, "DLH_SYNC_PROD_DATABASE")?;

    let local_host = env_or("DLH_SYNC_LOCAL_HOST", "127.0.0.1");
    let local_port = env_u16("DLH_SYNC_LOCAL_PORT", 3310);
    let local_user = env_or("DLH_SYNC_LOCAL_USER", "root");
    let local_pass = env_or("DLH_SYNC_LOCAL_PASSWORD", "root");
    let local_db = sanitize_db_name(
        &env_or("DLH_SYNC_LOCAL_DATABASE", "devleadhunter"),
        "DLH_SYNC_LOCAL_DATABASE",
    )?;

    let sync_mode = env_or("DLH_SYNC_USE_DOCKER", "auto").to_lowercase();
    let mysqldump_bin = resolve_client_bin("DLH_MYSQLDUMP_BIN", "mysqldump");
    let mysql_bin = resolve_client_bin("DLH_MYSQL_BIN", "mysql");
    let docker_bin = resolve_client_bin("DLH_DOCKER_BIN", "docker");
    let docker_image = env_or("DLH_SYNC_DOCKER_IMAGE", "mariadb:11");
    ...
    let local_connect_host = match &backend {
        ...
        SyncBackend::Docker { .. } => env_or("DLH_SYNC_LOCAL_DOCKER_HOST", "host.docker.internal"),
    };
    ...
    let stamp = std::time::SystemTime::now()
        ...
        .map(|d| d.as_millis())
        .unwrap_or(0);
    let prod_cnf = tmp.join(format!("devleadhunter_sync_prod_{stamp}.cnf"));
    let local_cnf = tmp.join(format!("devleadhunter_sync_local_{stamp}.cnf"));
    let dump_sql = tmp.join(format!("devleadhunter_sync_dump_{stamp}.sql"));

    write_client_cnf(&prod_cnf, &prod_host, prod_port, &prod_user, &prod_pass)?;
    write_client_cnf(&local_cnf, &local_connect_host, local_port, &local_user, &local_pass)?;

    let _cleanup = CleanupFiles(vec![prod_cnf.clone(), local_cnf.clone(), dump_sql.clone()]);
    ...
    run_mysql_exec(&backend, &local_cnf, &["-e", &create_sql]).map_err(|err| {
        format!("Failed to create local database:\n{err}\nTip: stop the local API container to free connections.")
    })?;

    run_mysql_import_stdin(&backend, &local_cnf, &local_db, &dump_sql).map_err(|err| {
        format!("Import into `{local_db}` failed:\n{err}\nTip: stop the local API container to free connections.")
    })?;
