    ...
    <div v-else class="relative flex min-h-screen items-center justify-center bg-[var(--app-bg)] px-4">
      ...
      <div class="w-full max-w-sm">
        ...
        <div class="mb-6">
          ...
          <h2 class="font-display mt-4 text-2xl font-semibold text-[var(--app-ink)]">Créer votre compte</h2>
          <p class="mt-1.5 text-sm text-[var(--app-ink-soft)]">
            Trouvez des artisans sans site web et transformez-les en clients.
          </p>
        </div>
        ...
        <form class="app-card space-y-4 p-6" @submit.prevent="handleSubmit">
          ...
          <div>
            ...
            <input
              ...
              :class="['input-field', nameError && 'border-[var(--app-red)]']"
            />
            <p v-if="nameError" class="mt-1 text-xs text-[var(--app-red)]">{{ nameError }}</p>
          </div>
...
const handleSubmit = async (): Promise<void> => {
  ...
  // Validate email
  if (!email.value) {
    emailError.value = 'Saisissez votre adresse email.'
    return
  }
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : 'La création du compte a échoué. Réessayez.'
