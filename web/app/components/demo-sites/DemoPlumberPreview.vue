<template>
  <div class="plumber-site" :class="{ 'plumber-site-embedded': embedded }" :style="cssVars">
    <!-- Nav -->
    <nav
      class="inset-x-0 top-0 z-50 border-b border-white/10 backdrop-blur-md"
      :class="embedded ? 'sticky' : 'fixed'"
      :style="{ backgroundColor: `${theme.secondary}ee` }"
    >
    ...
    >
      ...
      <div class="relative mx-auto max-w-6xl px-6 py-20 md:py-28">
        <span
          v-if="hero.badge"
          class="hero-fade-in inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-1.5 text-xs font-medium tracking-wider text-white uppercase"
        >
          <span class="pulse-dot h-2 w-2 rounded-full bg-emerald-400"></span>
          {{ hero.badge }}
        </span>
        <h1
          class="hero-fade-in hero-fade-in-delay-1 mt-6 max-w-3xl text-4xl leading-tight font-extrabold text-white md:text-6xl"
        >
    ...
    <section v-if="whyItems.length" class="px-6 py-20" :style="{ backgroundColor: theme.secondary }">
      <div class="mx-auto max-w-6xl">
        <h2 class="text-center text-3xl font-bold text-white md:text-4xl">
          {{ whyUs.heading || 'Pourquoi nous choisir ?' }}
        </h2>
        ...
        <div class="overflow-hidden rounded-3xl shadow-2xl" :style="{ background: contactGradient }">
          <div class="grid md:grid-cols-2">
            <div class="p-10 text-white md:p-14">
              ...
              <dl class="mt-8 space-y-4">
                <div v-if="contact.phone" class="flex items-center gap-3">
                  ...
                  <div>
                    <dt class="text-xs tracking-wide text-white/60 uppercase">Téléphone</dt>
                ...
                <div v-if="contact.email" class="flex items-center gap-3">
                  ...
                  <div>
                    <dt class="text-xs tracking-wide text-white/60 uppercase">Email</dt>
                ...
                <div v-if="contact.city" class="flex items-center gap-3">
                  <span class="flex h-10 w-10 items-center justify-center rounded-full bg-white/20">📍</span>
                  <div>
                    <dt class="text-xs tracking-wide text-white/60 uppercase">Zone</dt>
            ...
            <div class="flex flex-col justify-center bg-white p-10 md:p-14">
              <p class="text-sm font-semibold tracking-wide text-slate-500 uppercase">Devis gratuit</p>
              <p class="mt-2 text-2xl font-bold text-slate-900">Réponse sous 2 heures</p>
              <p class="mt-4 text-slate-600">
                Décrivez votre besoin — nous vous rappelons rapidement avec un devis transparent.
              </p>
              <a
                v-if="contact.phone"
...
const heroGradient = computed(() => `linear-gradient(135deg, ${theme.value.secondary} 0%, ${theme.value.primary} 100%)`)
