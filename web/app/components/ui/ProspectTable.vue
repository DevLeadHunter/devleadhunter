      ...
      <BaseTableTr
        ...
        :class="[
          ...
          isLockedForMe(prospect)
            ...
            : 'cursor-pointer',
          isBeingDragged(prospect) ? 'drag-reorder-slot' : '',
        ]"
      ...
      >
        <BaseTableTd v-if="reorderable" class="w-10 pr-0">
          <button
            ...
            @pointerdown="rowDrag.onGripPointerDown($event, prospect)"
          >
...
const rowDrag: UseDragToReorderReturn<Prospect> = useDragToReorder({
  axis: 'vertical',
  getContainer: (): HTMLElement | null => tableHostRef.value?.querySelector('table') ?? null,
  getOrder: (): Prospect[] => displayedProspects.value,
  keyOf: (prospect: Prospect): string => String(prospect.id),
  setDraftOrder: (order: Prospect[] | null): void => {
    draftOrder.value = order
  },
  setDraggedKey: (key: string | null): void => {
    draggedProspectKey.value = key
  },
  onCommit: emitReorderWhenChanged,
  ghostFrame: 'card',
})

const tableHostRef: Ref<HTMLElement | null> = ref(null)

/** Key (prospect id) of the row being dragged, kept until its ghost has landed; null otherwise. */
const draggedProspectKey: Ref<string | null> = ref(null)
...
/**
 * Whether this row is the one being dragged; it stays in the table as the landing slot until its ghost has landed.
 * @param prospect - Row to test.
 * @returns True while this prospect is being dragged or landing.
 */
function isBeingDragged(prospect: Prospect): boolean {
  return draggedProspectKey.value === String(prospect.id)
}

/**
 * Emit the dropped order when it differs from the saved one; a drop back in place changes nothing.
 * @param order - Prospects in their dropped order.
 */
function emitReorderWhenChanged(order: Prospect[]): void {
  const hasChanged: boolean = order.some(
