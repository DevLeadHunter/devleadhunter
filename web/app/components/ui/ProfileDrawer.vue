    ...
    <Transition name="drawer-panel">
      ...
      >
        ...
        <form id="profile-form" class="flex-1 space-y-4 overflow-y-auto px-5 py-4" @submit.prevent="handleSave">
          ...
          <div>
            <label class="text-muted mb-1.5 block text-xs font-medium" for="profile-email">Email</label>
            <input id="profile-email" v-model="form.email" type="email" required class="input-field" />
          </div>

          <div class="rounded-lg border border-[var(--app-line)] bg-[var(--app-bg)] px-3 py-2.5">
            <p class="flex items-center gap-2 text-xs text-[var(--app-ink-soft)]">
              <UIcon name="i-lucide-shield-check" class="h-3.5 w-3.5 shrink-0 text-[var(--app-green)]" />
              Vos identifiants restent privés — seuls le nom et l'email sont modifiables ici.
            </p>
          </div>
        </form>
