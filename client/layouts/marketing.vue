<template>
  <div class="min-h-screen bg-[#050505] text-[#f9f9f9]">
    <!-- Header -->
    <header class="sticky top-0 z-50 bg-[#050505]/95 backdrop-blur-lg border-b border-[#30363d]/30">
        ...
        <div class="flex items-center justify-between h-20">
          ...
          <NuxtLink :to="localePath('/')" class="flex items-center gap-2.5 group">
            <div class="w-5 h-5 flex items-center justify-center">
              <svg class="w-full h-full fill-current text-[#f9f9f9] group-hover:text-[#e4e4e4] transition-colors" viewBox="0 0 493 515" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
            ...
            <span class="text-base font-semibold text-[#f9f9f9] group-hover:text-[#e4e4e4] transition-colors">
              devleadhunter
            </span>
          </NuxtLink>
          ...
          <div class="hidden md:flex items-center gap-8">
            <a 
              ...
              class="text-base font-medium text-[#8b949e] hover:text-[#f9f9f9] transition-colors"
            >
              {{ $t('nav.features') }}
            </a>
            <a 
          ...
          <button 
            ...
            class="md:hidden text-[#8b949e] hover:text-[#f9f9f9] transition-colors p-2"
          >
            <i :class="isMobileMenuOpen ? 'fa-solid fa-times' : 'fa-solid fa-bars'" class="w-5 h-5"></i>
          </button>
        </div>
      </nav>
    </header>
    ...
    <Transition name="slide">
      ...
      >
        ...
        <!-- Sidebar -->
        <aside 
          class="fixed left-0 top-0 h-full w-full bg-[#1a1a1a] border-r border-[#30363d] z-[60] shadow-xl"
        >
        <div class="flex flex-col h-full">
          <!-- Header -->
          <div class="px-4 py-5 border-b border-[#30363d]">
            <div class="flex items-center justify-between">
              ...
              <button 
                @click="closeMobileMenu"
                class="text-[#8b949e] hover:text-[#f9f9f9] transition-colors p-2"
              >
          ...
          <nav class="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
            <a 
              href="#features" 
              @click.prevent="scrollToSection('#features'); closeMobileMenu()"
              class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-[#8b949e] hover:bg-[#21262d] hover:text-[#f9f9f9] transition-colors"
            >
    ...
    <!-- Footer -->
    <footer class="border-t border-[#30363d]/30 bg-[#1a1a1a] mt-32">
      <div class="container mx-auto px-4 md:px-6 lg:px-8 py-20">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-16">
          ...
          <div class="col-span-1 md:col-span-2">
            ...
            <p class="text-base text-[#8b949e] max-w-md leading-relaxed font-light">
          ...
          <!-- Links -->
          <div>
            <h3 class="text-sm font-semibold text-[#f9f9f9] uppercase tracking-wider mb-6">{{ $t('footer.product') }}</h3>
            <ul class="space-y-4">
              <li>
                <a href="#features" @click.prevent="scrollToSection('#features')" class="text-base text-[#8b949e] hover:text-[#f9f9f9] transition-colors font-light">
                  {{ $t('nav.features') }}
                </a>
              </li>
              <li>
                <a href="#pricing" @click.prevent="scrollToSection('#pricing')" class="text-base text-[#8b949e] hover:text-[#f9f9f9] transition-colors font-light">
                  {{ $t('nav.pricing') }}
                </a>
              </li>
            </ul>
          ...
          <div>
            <h3 class="text-sm font-semibold text-[#f9f9f9] uppercase tracking-wider mb-6">{{ $t('footer.account') }}</h3>
            <ul class="space-y-4">
              <li>
                <NuxtLink :to="localePath('/login')" class="text-base text-[#8b949e] hover:text-[#f9f9f9] transition-colors font-light">
                  {{ $t('nav.login') }}
                </NuxtLink>
              </li>
              <li>
                <NuxtLink :to="localePath('/signup')" class="text-base text-[#8b949e] hover:text-[#f9f9f9] transition-colors font-light">
                  {{ $t('nav.signup') }}
                </NuxtLink>
              </li>
            </ul>
        ...
        <div class="border-t border-[#30363d]/30 mt-16 pt-10">
          <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <p class="text-base text-[#8b949e] font-light">
            ...
            <div class="flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-6 w-full md:w-auto">
              <div class="flex items-center gap-2 w-full md:w-auto justify-start">
                <span class="text-sm text-[#8b949e] mr-2">Language:</span>
                <select
                  :value="currentLocale"
                  @change="switchLocale(($event.target as HTMLSelectElement).value)"
                  class="bg-[#1a1a1a] border border-[#30363d] text-[#f9f9f9] text-sm rounded-lg px-3 py-1.5 focus:outline-none focus:border-[#f9f9f9] transition-colors"
                >
                  <option
              ...
              <div class="flex flex-col md:flex-row gap-4 md:gap-8 items-start md:items-center w-full md:w-auto">
                <a href="#" class="text-base text-[#8b949e] hover:text-[#f9f9f9] transition-colors font-light">
                  {{ $t('footer.privacy') }}
                </a>
                <a href="#" class="text-base text-[#8b949e] hover:text-[#f9f9f9] transition-colors font-light">
                  {{ $t('footer.terms') }}
